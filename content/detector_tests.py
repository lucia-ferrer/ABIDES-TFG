...
from config.experiments import detector_experiments, params_to_str, DETECTOR_CLASS
...
def parse_args():
    ...
    parser.add_argument("--attack", default=-1,
                        help="The name of the attack to use. -1 for all (Default)"
    )
...
if __name__ == '__main__':
    ...
    # file name
    file_name = f"detector_tests_{args.norm}"
    ...
    logger = Logger(file_name, len(detectors_list) * len(attacks_list))
    # execute experiments
    results = pd.DataFrame()
    for detector_name, detector_params in detectors_list:
        defenses = {policy_id: Defense(norm=args.norm, detector=DETECTOR_CLASS[detector_name](**detector_params), recovery='cheat')
        ...
        for policy_id, attack_name, attack_params in attacks_list:
            ...
            results.to_csv(f"results/detection/{file_name}.csv", index=False)
